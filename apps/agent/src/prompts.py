...
INTEGRATION_PROMPT = (
    ...
    "    Idempotent bootstrap: installs gh CLI (apt-get) if missing\n"
    "    and configures git identity (user.email, user.name) so commits\n"
    "    don't fail. GITHUB_TOKEN/GH_TOKEN are auto-injected into every\n"
    "    sandbox shell exec — no separate auth step needed. Call this\n"
    "    once at the start of any github flow.\n"
    "- sandbox_gh(args, cwd?)\n"
    "    Run `gh <args>` inside the sandbox. Use for PR creation, repo\n"
    "    creation, PR review/merge, issue management, etc. The CLI is\n"
    "    auth'd via GH_TOKEN automatically.\n"
    "    Examples:\n"
    "      sandbox_gh('repo create kalil0321/demo --public --source=. "
    "--remote=origin --push', cwd='/home/daytona/demo')\n"
    "      sandbox_gh('pr create --title \"Add foo\" --body \"...\"', "
    ...
    "    4. sandbox_shell('git add -A && git commit -m \"fix: xyz\"',\n"
    "                     cwd='/home/daytona/<repo>')\n"
    "    5. sandbox_shell('git push -u origin fix-xyz',\n"
    "                     cwd='/home/daytona/<repo>')\n"
    "    6. sandbox_gh('pr create --title \"fix: xyz\" --body \"...\" --fill',\n"
    "                  cwd='/home/daytona/<repo>')\n"
    "- 'create a fresh repo and push my code':\n"
    "    1. sandbox_github_setup()\n"
    "    2. sandbox_shell('mkdir -p /home/daytona/<name> && cd /home/daytona/<name> "
    "&& git init')\n"
    "    3. sandbox_write_file(...) for each source file\n"
    "    4. sandbox_shell('git add -A && git commit -m \"init\"',\n"
    "                     cwd='/home/daytona/<name>')\n"
    "    5. sandbox_gh('repo create <user>/<name> --public --source=. "
    "--remote=origin --push', cwd='/home/daytona/<name>')\n"
    "\n"
